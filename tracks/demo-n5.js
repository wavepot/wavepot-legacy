...
export default async () => {
  ...
  var nopop = Nopop(.065,.002)

  var notes = scales['spanish']
    .map(n => n/1 + (1 + (rand()*2|0) * 12))

  var notesHz = notes.map(note)

  var chorder = await Chorder({
    notes, reverse: true,
    osc: Sqr, params: [256, true],
    octave: 5, speed: 1/4, voices: 4
  })
...
//
    ...
    // [222,'texture',.5,0,.7],
    // [666,'texture',4,0,2],
    // [666888,'texture',2,0,2.5],
    // [666888,'texture',1,0,2.5],
    // [666888999,'texture',1,0,0],
  ...
  return (t, f) => {
    t *= 1
    // t*=1.02;f*=1.02
    var kick = arp(t, 1/4, 52.3, 50, 20)

    var keys = chorder(t)
    keys = lpf
      .cut(5000 + perc(t%(1/16), 150, 30) + -lfo(1/8)*4200)
      .res(0.65)
      .sat(0.7)
      .update().run(keys)
    keys = perc(t%(1/4), 10, keys)
    keys = keys + biquad1.cut(800).res(1.5).gain(2).update().run(keys)

    var bass = bassOsc(slide(t, 1/8, 14, notesHz.map(n => n*2)))
    bass = bass * perc(t%(1/4),40,10)
    bass = diode
      .cut(.3 + 1 *
       perc(t%(1/4), 1.80, .10) // (t+(1/4))%(1/4),.27,.34) // magic
     + sin(t, 1/24)*.25 // magic
     + sin(t, 2)*.05
     // + (sin(t, 200)+1) *.05
       )
      .hpf(.00009)
      .res(.86)
      .run(bass * 4) //* //perc(t%(1/4),2,30) * .52

    bass = clip(bass, .79) // more magic
    var out = (0
      // + kick * .7
      + clip(kick * 1.5, .7)*.35
      + (
        + 0.3 * keys
        + 0.22 * bass
        + 0.7 * beats1(t, f % blockFrames)
      ) * (1 - perc(t%1/8, 20, 7.5)*.225)
    )
...
export var draw = t => {
  ...
  // a=(b,e=128)=>S(t*b/3)*e+e;x.fillStyle=R(a(7),a(3),a(2)+128,1);x.fillRect(a(5,960),a(10,540),40,20);x.drawImage(c,S(t)-.5,C(t)-.5,1922,1082)
