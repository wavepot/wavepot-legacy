...
export var dsp = async (context) => {
  ...
  var hall = await fetchSample('./impulses/220-R2_Reverb2.wav')
  ...
  var room = await fetchSample('./impulses/307-MediumRm.wav')
  // var impulse = await fetchSample('./impulses/313-RoomAmb.wav')
  // var impulse = await fetchSample('./impulses/315-LongCave.wav')

  var room_reverb = convolve(blockFrames, room)
  var hall_reverb = convolve(blockFrames, hall)

  var playSample = (sample, n) => sample[n % sample.length]
  ...
  return (t, n, floats) => {
    ...
    for (var i = 0; i < blockFrames; i++) {
      floats[i] = out[i]*.75 + r1[i]*.15 + r2[i]*.08
    }
    return floats
  }
}
