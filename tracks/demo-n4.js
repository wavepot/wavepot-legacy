...
//
  ...
  return (t, f) => {
    ...
    var keys = chorder(t)
    keys = lpf
      .cut(4000 + perc(t%(1/16), 150, 30) + -lfo(1/8)*3200)
      .res(0.35)
      .sat(0.4)
      .update().run(keys)
    keys = perc(t%(1/4), 10, keys)
    keys = keys + biquad1.cut(1500).res(2).gain(3).update().run(keys)

    var bass = bassOsc(slide(t, 1/16, 3, notesHz.map(n => n*2)))
    bass = bass * perc(t%(1/16),14,15) * .47
    bass = diode
      .cut(1.8 *
       perc((t+(1/4))%(1/4),.07,.14) // magic
     + sin(t, 4)*.025 // magic
     // + (sin(t, 2)) *.02
     // + (sin(t, 200)+1) *.05
       )
