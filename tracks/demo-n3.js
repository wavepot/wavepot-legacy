...
//
    ...
    // [222,'texture',2,0,.7],
    // [666,'texture',4,0,2],
    [666888,'texture',1,0,2],
    [666888999,'texture',5,0,2],
  ...
  return (t, f) => {
    ...
    bass = bass * perc(t%(1/4),14,15) * .47
    bass = diode
      .cut(2.2 *
       perc((t+(1/2))%(1),.0999,.0666) // magic
     + sin(t, 1/2)*.005 // magic
     // + (sin(t, 2)) *.02
     // + (sin(t, 4)+1) *.05
       )
      .hpf(.00009)
      .res(.97)
      .run(bass*1) //* //perc(t%(1/4),2,30) * .52

    bass = clip(bass, .39) // more magic
    var out = (0
      // + kick * .7
      + clip(kick * 3.5, .4)*.35
      + (
