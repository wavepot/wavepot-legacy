...
export var setup = async ({ blockFrames }) => {
  var vocal = await fetchSample('./samples/vocals/MissJudged_-_The_World_Ends.mp3')// http://ccmixter.org/content/MissJudged/MissJudged_-_The_World_Ends.mp3')
...
export var dsp = async (context) => {
  ...
  var vocal_part_1 = vocal.slice(0, blockFrames * 1.5)
  var vocal_part_2 = vocal.slice(blockFrames * 1.941, blockFrames * 1.941 + (blockFrames * 1.5))
  ...
  var mix = (t, n, floats) => {
    ...
    var vocals = resample(n, vocal_part_1, 1.9016, .20)
    // var vocals = resample(n, vocal_part_2, 1.9016, .20)
    // plot(vocals*20)

    var vocals_delay = delay
      .feedback(.95)
      // .delay(Math.floor((1/[2,2,2,3,4,16,32,64][Math.floor((t/2)%8)]) * blockFrames))
      .delay(Math.floor((1/[2,4,64,128,4,16,32,64][Math.floor((t*4)%8)]) * blockFrames))
      .run(0)
      // .run(vocals)
