...
export default async () => {
  ...
  return (t, f) => {
    ...
    var keys2 = chorder2(t) * perc(t%(1/2), 15, 300)*.01
    keys2 = lpf
      .cut(800 + perc(t%(1/2), 15, 300) + -lfo(2)*400)
      .res(0.75)
      .sat(0.8)
      .update().run(keys2)

    var bass = bassOsc(slide(t/4, 1/4, 8, notesHz.map(n => n*2)))
    bass = bass * perc(t%(1/2),10,25) * .17
    bass = diode
      .cut(2.10 *
       perc((t+(1))%(1.5),
       1,.04) // magic
