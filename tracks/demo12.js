...
export default async () => {
  ...
  return (t, f) => {
    ...
    var out = (0
      ...
      + .7 * keys
      + 0.11 * bass
      + beats1(t, f % blockFrames)
    )

    // eq
    out = out - biquad.cut(150).res(2).gain(3).update().run(out)*.3
...
export var draw = t => {
  // dna
  // for(i=0;i<1100;i++){d=C(t+1*i),s=i==0?2920:9-d*49;x.fillStyle=R(i,i,i,0.1);x.fillRect(S(t*2.5+3*i)*220*S(C(t)+i%50)+960*(i==0?-1:1),i,s,s);};x.fillStyle='transparent'
