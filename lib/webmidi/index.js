...
export default async function MIDI(port = 0, channel = 0, IO) {
  ...
  const input = Array.from(inputs)[port]
  input.onmidimessage = onMIDIMessage(channel)
  return (key) => (CTRL_KEY_MAP[key] || 0)
}