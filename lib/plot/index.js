...
export default function Plot (frames, opts) {
  function draw (samples) {
    const os = new Oscilloscope(opts)
    samples.os = os
    os.render(samples)
    os.event = () => {
      plots.forEach(p => {
  ...
  return function plot (value = null) {
    if (value === null) {
      id = -1
      frame++
    } else {
      id++
      if (!(id in plots)) {
        plots[id] = new Float32Array(frames)
        const [ line, column ] = (new Error()).stack.split('\n')[2].split(':').slice(3)
        plots[id].pos = { line: parseInt(line), column: parseInt(column) }
      }
      plots[id][frame] = value
